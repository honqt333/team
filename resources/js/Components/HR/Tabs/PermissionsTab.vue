...
<template>
    ...
    <div v-else-if="!hasUser" class="text-center py-12">
        <div
            class="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-8 max-w-lg mx-auto border border-dashed border-gray-300 dark:border-gray-700">
            <div
                class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-violet-100 dark:bg-violet-900/30 mb-6">
                <svg class="h-8 w-8 text-violet-600 dark:text-violet-400" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
    ...
    <form v-else @submit.prevent="submit" class="space-y-8 animate-fade-in-up">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
            ...
            <div v-if="availableRoles.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <label v-for="role in availableRoles" :key="role.id"
                    ...
                    <div class="flex-1">
                        <span
                            class="block text-base font-semibold text-gray-900 dark:text-white mb-1 group-hover:text-violet-700 dark:group-hover:text-violet-300 transition-colors">
                    ...
                    <div v-if="form.roles.includes(role.id)" class="absolute top-4 end-4 text-violet-500">
            ...
            <div v-else
                class="text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-dashed border-gray-200 dark:border-gray-700">
        ...
        <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="px-2 py-0.5 text-xs font-bold bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded-full">
                                    {{ totalActivePermissions }} {{ t('permissions.active', 'مفعّلة') }}
                                </span>
                            </div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                <span class="text-violet-600 dark:text-violet-400 font-medium">{{ rolePermissions.length
                                    }}</span> {{ t('permissions.from_roles', 'من الأدوار') }}
                                •
                                <span class="text-indigo-600 dark:text-indigo-400 font-medium">{{
                                    form.permissions.length }}</span> {{ t('users.direct_permissions', 'مباشرة') }}
                            </p>
                        </div>
                    </div>
            ...
            <div class="divide-y divide-gray-100 dark:divide-gray-700">
                <div v-for="(perms, moduleName) in filteredGroupedPermissions" :key="moduleName" class="group">
                    <!-- Module Header -->
                    <div class="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer"
                        ...
                        <div class="flex items-center gap-3">
                            <button type="button"
                                ...
                                <svg class="w-5 h-5 text-gray-400 transition-transform duration-200"
                                    :class="{ 'rotate-180': activeModules.includes(moduleName) }" fill="none"
                            ...
                            <span class="px-2 py-0.5 text-xs font-medium rounded-full" :class="getModuleActiveCount(moduleName) > 0
                            ...
                            <div v-for="permission in perms" :key="permission"
                                ...
                                <div class="flex flex-col pe-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span v-if="isPermissionFromRole(permission)"
                                    ...
                                    <span class="text-[10px] text-gray-400 font-mono mt-0.5 truncate max-w-[200px]"
                                        :title="permission">
                                        {{ permission }}
                                    </span>
                                </div>
                                <Toggle v-if="!isPermissionFromRole(permission)" v-model:checked="form.permissions"
                                    :value="permission" />
        ...
        <!-- Actions -->
        <div
            class="sticky bottom-0 z-10 bg-gray-50/95 dark:bg-gray-900/95 backdrop-blur-sm p-4 -mx-4 -mb-4 flex justify-end border-t border-gray-200 dark:border-gray-700">
            <button type="submit"

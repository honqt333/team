<template>
    ...
    >
        ...
        <div
            class="flex justify-between items-start pb-4 mb-6 border-b-2"
            :class="isModern ? 'border-dashed border-gray-300' : 'border-gray-800'"
        >
            <div class="w-1/3">
                <h1 class="text-2xl font-bold text-gray-900 mb-1">
                    {{ centerData.name || 'مركز اور كارز' }}
                </h1>
                ...
                <p v-if="centerData.phone" class="text-sm text-gray-600 mt-2">
                    {{ centerData.phone }}
                </p>
            </div>
            ...
            <div class="w-1/3 flex flex-col items-center justify-center">
                <div v-if="visualSettings.show_logo" class="mb-3">
                    <img
                        v-if="centerData.logo_url || centerData.logo"
                        :src="centerData.logo_url || centerData.logo"
                        alt="Logo"
                        class="w-24 h-24 object-contain"
                    />
                    <div
                        v-else
                        class="w-24 h-24 bg-gray-100 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center text-gray-400 text-xs font-bold"
                    >
                        [Logo]
                    </div>
                </div>
                <h2
                    ...
                    :class="
                        isModern
                            ? 'bg-slate-900 text-white px-6 py-1.5 rounded-full shadow-sm'
                            : 'bg-gray-100 border border-gray-200 px-4 py-1 rounded shadow-sm text-gray-900'
                    "
            ...
            <div class="w-1/3 flex flex-col" :class="isRtl ? 'items-end' : 'items-start'">
                <div
                    v-if="documentSettings.show_qr_code"
                    class="w-20 h-20 bg-gray-100 border border-gray-300 p-1 mb-2"
                >
                    <img
                        v-if="data.qr_code_url"
                ...
                <div class="text-sm font-medium" :class="isRtl ? 'text-left' : 'text-right'">
                    <p class="text-gray-800 font-bold" :class="isRtl ? 'text-right' : 'text-left'">
                        {{ $t('common.code') }}:
                        <span class="font-mono">{{ data.code || 'T202620001' }}</span>
                    </p>
                    <p class="text-gray-600 mt-0.5" :class="isRtl ? 'text-right' : 'text-left'">
                        {{ $t('work_orders.print_view.date') }}:
        ...
        <div
            :class="
                ['parts_invoice', 'purchase_invoice', 'purchase_return'].includes(documentType)
                    ? 'block mb-6'
        ...
        >
            <div
                ...
                :class="
                    isModern ? 'border-0 bg-slate-50/50 rounded-2xl' : 'border-gray-200 rounded-lg'
                "
            >
                <h3
                    ...
                    :class="[
                        ...
                        isRtl
                            ? 'right-0 border-l rounded-bl-lg' +
                              (isModern ? ' rounded-tr-2xl' : ' rounded-tr-lg')
                            : 'left-0 border-r rounded-br-lg' +
                              (isModern ? ' rounded-tl-2xl' : ' rounded-tl-lg'),
                    ]"
                    :style="isModern ? { backgroundColor: primaryColor } : {}"
                >
                    {{
                        ['purchase_invoice', 'purchase_return'].includes(documentType)
                            ? isRtl
                                ? 'المورد'
                                : 'Supplier'
                ...
                <div class="grid grid-cols-2 gap-2 text-sm text-gray-700 mt-2">
                    <p>
                        ...
                        {{ data.customer?.name || (isRtl ? 'احمد الزهراني' : 'Ahmad Alzahrani') }}
                    </p>
                    <p>
                        <span class="text-gray-500">{{ $t('common.phone') }}:</span>
                        <span dir="ltr">{{ data.customer?.phone || '+966 555 785 658' }}</span>
                    </p>
                    <p
                        v-if="
                            documentSettings.show_customer_address !== false &&
                            data.customer?.address
                        "
                        class="col-span-2"
                    >
                        <span class="text-gray-500">{{ $t('common.address') }}:</span>
                        {{ data.customer.address }}
                    </p>
                    <p v-if="isTaxEnabled() && data.customer?.tax_number" class="col-span-2">
                        <span class="text-gray-500">
                            {{ $t('company_profile.profile.vat_number') }}:
                        </span>
                ...
                <div class="grid grid-cols-2 gap-2 text-sm text-gray-700 mt-2 items-center">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-bold flex items-center gap-1.5">
                            <img
                                v-if="data.vehicle?.make_logo"
                                :src="data.vehicle.make_logo"
                                alt="Make Logo"
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <SaudiPlateDisplay
                            v-if="data.vehicle?.plate"
                            :plate-number="data.vehicle.plate"
                            size="sm"
                        />
                        <span v-else class="font-bold">-</span>
                    </div>
                    <p v-if="data.vehicle?.color">
                        <span class="text-gray-500">{{ $t('work_orders.print_view.color') }}:</span>
                        {{ getLocalizedColor(data.vehicle.color, isRtl) }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="mb-6">
            <!-- Scenario 1: Receipt -->
            <div
                v-if="documentType === 'receipt'"
                class="bg-gray-50/50 p-6 rounded-lg border border-gray-200 leading-loose text-base text-gray-800"
            >
                <p>
                    {{ $t('work_orders.print_view.received_from') }}:
                    <strong class="underline underline-offset-4">
                        {{ data.customer?.name || (isRtl ? 'احمد الزهراني' : 'Ahmad Alzahrani') }}
                    </strong>
                </p>
                <p class="mt-2">
                    {{ $t('work_orders.print_view.amount_of') }}:
                    <strong class="font-mono">{{ formatCurrency(data.amount || 250) }}</strong>
                </p>
                <p class="mt-2 text-gray-600">
                    ...
                    <span>
                        {{
                            data.notes ||
                            (isRtl
                                ? 'دفعة مقدمة لصيانة السيارة المذكورة أعلاه وبدء أعمال الفحص والبرمجة.'
                                : 'Down payment for maintenance of the above-mentioned vehicle and starting inspection and programming.')
                        }}
                    </span>
                </p>
            </div>
                ...
                <div v-if="services.length > 0">
                    ...
                    >
                        <thead>
                            <tr
                                class="text-white"
                                :style="{ backgroundColor: isModern ? primaryColor : '#1f2937' }"
                            >
                                ...
                                <th
                                    v-if="hasServicesVat"
                                    class="p-2 border border-gray-700 w-20 text-center"
                                >
                                    {{ $t('common.vat') }}
                                </th>
                                <th class="p-2 border border-gray-700 w-22 text-center">
                                    {{ $t('common.total') }}
                                </th>
                            </tr>
                        </thead>
                        <tbody class="text-gray-800">
                            ...
                            >
                                <td class="p-2 border border-gray-200 text-center text-gray-500">
                                    {{ index + 1 }}
                                </td>
                                <td class="p-2 border border-gray-200 font-medium">
                                    ...
                                    <span
                                        v-if="item.description && item.service_name"
                                        class="block text-[10px] text-gray-400 mt-0.5 font-normal whitespace-pre-wrap"
                                    >
                                        {{ item.description }}
                                    </span>
                                </td>
                                ...
                                <!-- Discount -->
                                <td
                                    class="p-2 border border-gray-200 text-center font-mono text-red-600"
                                >
                                    {{
                                        lineDiscountDisplay(item) > 0
                                            ? '- ' + formatRawPrice(lineDiscountDisplay(item))
                                            : '-'
                                    }}
                                </td>
                                <!-- Amount -->
                                ...
                                <td
                                    v-if="hasServicesVat"
                                    class="p-2 border border-gray-200 text-center font-mono text-gray-500"
                                >
                                    {{ formatRawPrice(srvLineVat(item)) }}
                                </td>
                                <!-- Total incl. VAT -->
                                <td
                                    class="p-2 border border-gray-200 text-center font-bold font-mono"
                                >
                        ...
                        <tfoot>
                            <tr
                                class="font-bold bg-gray-50 text-gray-700 border-t-2 border-gray-300"
                            >
                                <td
                                    :colspan="hasServicesVat ? 6 : 5"
                                    class="p-2 border border-gray-200 text-center"
                                >
                                    {{
                                        documentType === 'parts_invoice'
                                            ? isRtl
                                ...
                                <td
                                    class="p-2 border border-gray-200 text-center font-mono font-bold"
                                >
                                    {{
                                        formatRawPrice(
                                            services.reduce((s, i) => s + srvLineTotal(i), 0)
                                        )
                                    }}
                                </td>
                            </tr>
                        ...
                        {{
                            data.is_company
                                ? isRtl
                                    ? 'البنود'
                                    : 'Items'
                                : ['parts_invoice', 'purchase_invoice', 'purchase_return'].includes(
                                        documentType
                                    )
                                  ? isRtl
                                      ? 'قطع الغيار'
                        ...
                        <thead>
                            ...
                            >
                                ...
                                <th
                                    v-if="
                                        [
                                            ...
                                            'purchase_invoice',
                                            'purchase_return',
                                        ].includes(documentType) && !data.is_company
                                    "
                                    class="p-2 border border-gray-700 w-24 text-center"
                                >
                                ...
                                <th class="p-2 border border-gray-700 w-20 text-center">
                                    {{
                                        isRtl
                                            ? ['purchase_invoice', 'purchase_return'].includes(
                                                  documentType
                                              ) && !data.is_company
                                                ? 'الكمية المستلمة'
                                                : 'الكمية'
                        ...
                        <tbody class="text-gray-800">
                            ...
                            >
                                ...
                                <td
                                    ...
                                    "
                                    class="p-2 border border-gray-200 text-center font-mono"
                                >
                                    {{
                        ...
                        <tfoot>
                            ...
                            >
                                ...
                                >
                                    {{
                                        formatRawPrice(
                                            parts.reduce((s, i) => s + partLineTotal(i), 0)
                                        )
                                    }}
                                </td>
                            </tr>
                    ...
                    >
                        <thead>
                            ...
                            >
                                ...
                                <th class="p-2 border border-gray-700">
                                    {{ $t('work_orders.print_view.method') }}
                                </th>
                                <th class="p-2 border border-gray-700">
                                    {{ $t('work_orders.print_view.reference') }}
                                </th>
                                <th class="p-2 border border-gray-700">
                                    {{ $t('common.details') }}
                                </th>
                                <th class="p-2 border border-gray-700 w-32 text-center">
                                    {{ $t('work_orders.print_view.amount') }}
                                </th>
                            </tr>
                        </thead>
                        <tbody class="text-gray-800">
                            ...
                            >
                                ...
                                <td
                                    class="p-2 border border-gray-200 font-bold"
                                    :class="payment.type === 'bad_debt' ? 'text-amber-700' : ''"
                                >
                                    {{
                                ...
                                <td class="p-2 border border-gray-200 font-mono" dir="ltr">
                                    {{ payment.reference || '-' }}
                                </td>
                            ...
                            <tr
                                ...
                                class="font-bold bg-gray-50 border-t border-gray-300"
                            >
                                <td colspan="5" class="p-2 border border-gray-200 text-center">
                                    {{ isRtl ? 'إجمالي المدفوعات:' : 'Total Payments:' }}
                                </td>
                                <td
                                    class="p-2 border border-gray-200 text-center font-mono text-gray-900"
                                >
                            ...
                            <tr v-if="!data.payments || data.payments.length === 0">
                                <td
                                    colspan="6"
                                    class="p-4 border border-gray-200 text-center text-gray-400"
                                >
            ...
            <div v-else-if="documentType === 'condition_report'" class="space-y-6">
                ...
                <div class="grid grid-cols-3 gap-4 text-sm">
                    <div class="bg-gray-50/50 p-3 rounded-xl border border-gray-100 flex flex-col">
                        <span class="text-xs text-gray-500 mb-0.5">
                            {{ $t('work_orders.print_view.entry_date') }}
                        </span>
                        <span class="font-bold text-gray-800 text-sm">
                            {{ formatDate(data.entry_date || data.created_at) }}
                        </span>
                    </div>
                    <div class="bg-gray-50/50 p-3 rounded-xl border border-gray-100 flex flex-col">
                        <span class="text-xs text-gray-500 mb-0.5">
                            {{ $t('work_orders.print_view.odometer') }}
                        </span>
                        <span class="font-bold text-gray-800 text-sm" dir="ltr">
                            {{
                                data.mileage
                                    ? Number(data.mileage).toLocaleString() +
                                      (isRtl ? ' كم' : ' km')
                                    : '-'
                            }}
                        </span>
                    ...
                    <div class="bg-gray-50/50 p-3 rounded-xl border border-gray-100 flex flex-col">
                        <span class="text-xs text-gray-500 mb-0.5">
                            {{ $t('work_orders.print_view.fuel_level') }}
                        </span>
                        <span class="font-bold text-gray-800 text-sm">
                            {{ data.fuel_level ? data.fuel_level + '%' : '-' }}
                        </span>
                    </div>
                </div>
                ...
                <div class="mb-6">
                    <div class="bg-white rounded-xl p-4 border border-gray-200">
                        <svg viewBox="0 0 400 300" class="w-full h-auto max-w-lg mx-auto">
                            <image
                                href="/images/vehicle-diagram.png"
                                x="0"
                                y="0"
                                width="400"
                                height="300"
                        ...
                        <!-- Legend -->
                        <div
                            class="flex items-center justify-center gap-6 mt-4 text-sm border-t pt-4"
                        >
                            <div class="flex items-center gap-2">
                                <span class="w-4 h-4 rounded-full bg-red-500"></span>
                                <span class="text-gray-600">
                                    {{ $t('work_orders.print_view.damage') }}
                                </span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="w-4 h-4 rounded-full bg-blue-500"></span>
                                <span class="text-gray-600">
                                    {{ $t('work_orders.print_view.scratch') }}
                                </span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="w-4 h-4 rounded-full bg-gray-500"></span>
                                <span class="text-gray-600">
                                    {{ $t('work_orders.print_view.note') }}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div v-if="damageMarks.length > 0" class="mb-6">
                    <h4 class="font-bold text-gray-855 mb-2">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        ...
                        >
                            ...
                            <span class="text-gray-700 font-medium">
                                {{ mark.description || $t('work_orders.print_view.na') }}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div v-else class="space-y-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-800 mb-2 flex items-center gap-2">
                        {{
                            documentType === 'work_order'
                                ? $t('work_orders.print_view.required_services')
                                : documentType === 'parts_invoice'
                                  ? isRtl
                                      ? 'قطع الغيار'
                                      : 'Spare Parts'
                    ...
                    >
                        <thead>
                            <tr
                                class="text-white"
                                :class="isModern ? '' : 'bg-gray-800'"
                                :style="{ backgroundColor: isModern ? primaryColor : '#1f2937' }"
                            >
                                <th class="p-2 border border-gray-700 w-8 text-center">#</th>
                                <th class="p-2 border border-gray-700">
                                    {{ $t('work_orders.print_view.service') }}
                                </th>
                                <!-- Status / Dates columns — work_order only -->
                                <template v-if="documentType === 'work_order'">
                                    <th class="p-2 border border-gray-700 w-24 text-center">
                                        {{ $t('work_orders.print_view.status') }}
                                    </th>
                                    <th class="p-2 border border-gray-700 w-24 text-center">
                                ...
                                <th
                                    v-if="showPricingColumns"
                                    class="p-2 border border-gray-700 w-28 text-center"
                                >
                                    {{ $t('common.total') }}
                                </th>
                            </tr>
                        ...
                        <tbody class="text-gray-800">
                            ...
                            >
                                ...
                                <td class="p-2 border border-gray-200 font-medium">
                                    ...
                                    <span
                                        v-if="item.description && item.service_name"
                                        class="block text-xs text-gray-500 mt-0.5 font-normal"
                                    >
                                        {{ item.description }}
                                    </span>
                                ...
                                <template v-if="documentType === 'work_order'">
                                    ...
                                    <!-- Start Date -->
                                    <td
                                        class="p-2 border border-gray-200 text-center font-mono text-xs text-gray-600"
                                    >
                                        {{ item.started_at ? formatDate(item.started_at) : '-' }}
                                    </td>
                                    <!-- Due Date -->
                                    <td
                                        class="p-2 border border-gray-200 text-center font-mono text-xs"
                                        :class="
                                            item.due_date &&
                                ...
                                >
                                    {{
                                        formatRawPrice(
                                            item.unit_price * (item.qty || 1) - (item.discount || 0)
                                        )
                                    }}
                                </td>
                            </tr>
                ...
                <div
                    v-if="
                        documentType === 'work_order' &&
                        (data.customer_complaint || data.initial_assessment)
                    "
                    class="grid gap-3"
                    :class="
                        data.customer_complaint && data.initial_assessment
                ...
                >
                    <!-- Complaint -->
                    <div
                        v-if="data.customer_complaint"
                        class="rounded-xl border border-amber-200 bg-amber-50/60 p-3"
                    >
                        <p
                            class="text-[10px] font-black text-amber-600 uppercase tracking-widest mb-1.5 flex items-center gap-1"
                        >
                    ...
                    <!-- Assessment -->
                    <div
                        v-if="data.initial_assessment"
                        class="rounded-xl border border-blue-200 bg-blue-50/60 p-3"
                    >
                        <p
                            class="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1.5 flex items-center gap-1"
                        >
                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div
            v-if="showPricingColumns && documentType !== 'receipt' && documentType !== 'payments'"
            class="flex justify-between items-start mb-12"
        >
            <div class="w-1/2 pr-4 text-sm text-gray-500">
                <div
                    v-if="
                        documentSettings.print_terms &&
                        (documentSettings.terms?.length > 0 ||
                            (previewMode && dummyTerms.length > 0))
                    "
                >
                    <h4 class="text-xs font-bold text-gray-700 mb-1">
                        {{ $t('work_orders.print_view.terms_conditions') }}:
                    </h4>
                    <ol
                        class="list-decimal list-inside text-[10px] text-gray-500 space-y-1 leading-normal pr-2"
                    >
                        <li
                            v-for="(term, idx) in documentSettings.terms?.length > 0
                                ? documentSettings.terms
                                : dummyTerms"
                            :key="idx"
                        >
                            {{
            ...
            <div class="w-1/2 lg:w-1/3">
                <table class="w-full text-sm">
                    <tbody>
                        <tr
                            v-if="documentType !== 'payments'"
                            class="text-gray-500 border-b border-gray-200"
                        >
                            <td class="py-1 px-2">{{ $t('quotes.form.subtotal') }}:</td>
                            <td class="py-1 px-2 text-left font-mono">
                                {{ formatCurrency(totals.subtotal) }}
                            </td>
                        </tr>
                        <tr
                            v-if="documentType !== 'payments' && totals.discount > 0"
                            class="text-red-500 border-b border-gray-200"
                        >
                        ...
                        <tr
                            v-if="documentType !== 'payments' && totals.vat > 0"
                            class="text-gray-500 border-b border-gray-200"
                        >
                        ...
                        >
                            ...
                            <td class="py-2 px-2 text-left text-gray-900 font-mono">
                                {{ formatCurrency(totals.total) }}
                            </td>
                        </tr>
                        <tr
                            v-if="!['quotation'].includes(documentType)"
                            class="text-gray-500 border-b border-gray-200"
                        >
                            <td class="py-1 px-2 font-bold">
                                {{ isRtl ? 'المبلغ المدفوع:' : 'Paid Amount:' }}
                            </td>
                            <td class="py-1 px-2 text-left font-mono font-bold">
                                {{ formatCurrency(totals.paid - (data.bad_debt || 0)) }}
                            </td>
                        </tr>
                        <tr
                            v-if="!['quotation'].includes(documentType) && (data.bad_debt || 0) > 0"
                            class="text-amber-600 border-b border-gray-200"
                        >
                            <td class="py-1 px-2 font-bold">
                                {{ isRtl ? 'الديون المعدومة:' : 'Bad Debt:' }}
                            </td>
                            <td class="py-1 px-2 text-left font-mono font-bold">
                                {{ formatCurrency(data.bad_debt) }}
                            </td>
                        </tr>
                        <tr
                            v-if="!['quotation'].includes(documentType)"
                            class="text-gray-700 border-b border-gray-200 bg-gray-50/50 font-black"
                        >
                            <td class="py-1.5 px-2">{{ isRtl ? 'الباقي:' : 'Remaining:' }}</td>
                            <td class="py-1.5 px-2 text-left font-mono">
                ...
                <div
                    v-if="centerData.iban && documentSettings.show_iban"
                    class="bg-gray-50 p-2 rounded border border-gray-200 mt-2 text-[9px] text-gray-500 leading-normal font-mono"
                >
                    <span class="block font-bold text-gray-600">
                        {{ $t('company_profile.profile.iban') }}:
                    </span>
                    {{ centerData.iban }}
                </div>
            </div>
        ...
        <div
            v-else-if="
                documentSettings.print_terms &&
                (documentSettings.terms?.length > 0 || (previewMode && dummyTerms.length > 0))
            "
            class="mb-12"
        >
            ...
            <ol
                class="list-decimal list-inside text-xs text-gray-500 space-y-1.5 leading-relaxed pr-2"
            >
                <li
                    v-for="(term, idx) in documentSettings.terms?.length > 0
                        ? documentSettings.terms
        ...
        <div class="mt-auto pt-8 border-t-2 border-gray-800">
            <div
                class="grid gap-8 text-center text-sm font-bold text-gray-800 mb-8 relative"
                :class="getSignatureGridClass(visibleSignatures.length || 2)"
            >
                <!-- Official Stamp positioned absolutely over signatures grid -->
                <div
                    v-if="
                        documentSettings.show_stamp &&
                        (previewMode || centerData.stamp_url || visualSettings.stamp_url)
                    "
                    class="absolute inset-0 flex items-center justify-center opacity-80 pointer-events-none -rotate-12 z-10 select-none"
                >
                    <img
                        v-if="centerData.stamp_url || visualSettings.stamp_url"
                        :src="centerData.stamp_url || visualSettings.stamp_url"
                        class="w-28 h-28 object-contain"
                    />
                    <!-- Fallback premium stamp design in SVG if in preview mode and image not provided -->
                    <svg
                        v-else-if="previewMode"
                        class="w-28 h-28 text-emerald-600/80 print:hidden"
                        fill="none"
                        viewBox="0 0 100 100"
                        stroke="currentColor"
                    >
                ...
                >
                    ...
                    <div
                        v-if="sig.url"
                        class="h-16 flex items-end justify-center mb-2"
                    ...
                    <div
                        v-else-if="
                            isClientSignature(isRtl ? sig.name_ar : sig.name_en || sig.name_ar) &&
                            (data.reception_signature || data.delivery_signature)
                        "
                        class="h-16 flex items-center justify-center mb-2"
                    >
                        <img
                            :src="
                                '/storage/' + (data.reception_signature || data.delivery_signature)
                            "
            ...
            <!-- Footer Text -->
            <div
                v-if="visualSettings.footer_text"
                class="text-center text-[10px] text-gray-400 leading-normal border-t border-gray-100 pt-2 mt-4"
            >
...
function lineSubtotalDisplay(item) {
    if (
        item.line_total_excl_tax !== undefined &&
        item.line_total_excl_tax !== null &&
        item.line_total_excl_tax !== 0
    ) {
        return showExclusive.value
            ? Number(item.line_total_excl_tax)
            : Number(item.line_total_incl_tax);
    }
    ...
    if (props.data.pricing_mode_snapshot === 'inclusive') {
        if (showExclusive.value) {
            const rate =
                isTaxEnabled() && item.is_taxable !== false
                    ? item.tax_rate_snapshot != null
                        ? item.tax_rate_snapshot / 100
                        : 0.15
                    : 0;
...
const hasServicesVat = computed(() => services.value.some((i) => srvLineVat(i) > 0));
const hasPartsVat = computed(() => parts.value.some((i) => partLineVat(i) > 0));

const getPartsFooterColspan = computed(() => {
    let base = 6;
    if (hasPartsVat.value) base += 1;
    if (
        ['parts_invoice', 'purchase_invoice', 'purchase_return'].includes(props.documentType) &&
...
const getColorValue = (color) =>
    ({ red: '#ef4444', blue: '#3b82f6', gray: '#6b7280' })[color] || '#ef4444';
...
function getDocTypeTitle(type) {
    ...
    const titlesAr = {
        invoice: taxActive ? 'فاتورة ضريبية' : 'فاتورة',
        proforma_invoice: 'فاتورة أولية',
        quotation: 'عرض سعر',
        parts_invoice: taxActive ? 'فاتورة قطع غيار ضريبية' : 'فاتورة قطع غيار',
        ...
        purchase_invoice: taxActive ? 'فاتورة مشتريات ضريبية' : 'فاتورة مشتريات',
        purchase_return: taxActive ? 'فاتورة مرتجع مشتريات ضريبية' : 'فاتورة مرتجع مشتريات',
    };
    const titlesEn = {
        invoice: taxActive ? 'Tax Invoice' : 'Invoice',
        proforma_invoice: 'Proforma Invoice',
        quotation: 'Quotation',
        parts_invoice: taxActive ? 'Tax Parts Invoice' : 'Parts Invoice',
        ...
        purchase_invoice: taxActive ? 'Tax Purchase Invoice' : 'Purchase Invoice',
        purchase_return: taxActive ? 'Tax Purchase Return Invoice' : 'Purchase Return Invoice',
    };
    if (isRtl.value) {
...
const dummyItems = [
    {
        service_name: 'تغيير اقمشة',
        description: 'قطع غيار أصلية مع ضبط الميزانية والفرامل',
        qty: 1,
        unit_price: 172.5,
    },
];
...
const visibleSignatures = computed(() => {
    const sigs =
        props.documentSettings?.signatures?.length > 0
...
const totals = computed(() => {
    ...
    if (props.data.total_incl_tax !== undefined && props.data.total_incl_tax !== null) {
        ...
        const total = taxEnabled
            ? Number(props.data.total_incl_tax || 0)
            : Number(props.data.total_excl_tax || props.data.total_incl_tax || 0);
        ...
        const balance = Number(
            props.data.balance !== undefined ? props.data.balance : Math.max(total - paid, 0)
        );
    ...
    items.forEach((item) => {
        ...
        if (isInclusive) {
            ...
            lineDisc =
                taxEnabled && item.is_taxable !== false ? itemDiscount / (1 + rate) : itemDiscount;
        } else {
    ...
    const balance =
        props.data.balance !== undefined ? props.data.balance : Math.max(totalAmt - paid, 0);
...
function isClientSignature(name) {
    ...
    return (
        n.includes('عميل') || n.includes('العميل') || n.includes('customer') || n.includes('client')
    );
}

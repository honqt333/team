<template>
    <BaseModal :show="show" @close="$emit('close')" size="2xl">
        ...
        <form @submit.prevent="submitForm" class="space-y-5">
            <!-- ═══════════════════════════════════════════════════════
                 CREATE MODE — Catalog selection dropdown
                 ═══════════════════════════════════════════════════════ -->
            <template v-if="!service?.id">
                <div class="flex gap-3">
                    <div class="flex-1">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                            {{ $t('packages.select_from_catalog') || 'اختر باقة من الكتالوج الموحد' }}
                        </label>
                        <SearchableSelect
                            v-model="selectedCatalogPackageId"
                            :options="otherBranchesPackages"
                            :option-label="p => p.name_ar || p.name_en"
                            option-value="id"
                            :placeholder="$t('packages.select_package_placeholder') || 'اختر باقة...'"
                            label=""
                        />
                    </div>
                ...
                <div v-if="selectedCatalogPackage"
                    class="p-4 rounded-xl bg-indigo-50/60 dark:bg-indigo-900/10 border border-indigo-200/60 dark:border-indigo-800/30 space-y-3"
                >
                    <div class="flex items-start justify-between gap-2">
                        <div>
                            <h4 class="font-bold text-gray-900 dark:text-white text-sm">
                                {{ selectedCatalogPackage.name_ar }}
                            </h4>
                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ selectedCatalogPackage.name_en }}</p>
                        </div>
                    </div>
                    <p v-if="selectedCatalogPackage.description_ar"
                        class="text-xs text-gray-500 dark:text-gray-400 border-t border-indigo-200/50 dark:border-indigo-800/30 pt-2"
                    >
                        {{ selectedCatalogPackage.description_ar }}
                    </p>
                    
                    <!-- Items list preview -->
                    <div class="border-t border-indigo-200/50 dark:border-indigo-800/30 pt-2">
                        <h5 class="text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">{{ $t('packages.items') }}:</h5>
                        <ul class="space-y-1 text-xs text-gray-600 dark:text-gray-400">
                            <li v-for="item in selectedCatalogPackage.items" :key="item.id" class="flex items-center gap-2">
                            ...
                            <div v-for="(item, index) in form.items" :key="index"
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500">{{ getServicePrice(item.id) }} {{ $t('common.currency_sar') }} / {{ $t('common.unit') }}</p>
                                    </div>
                                </div>
            ...
            <!-- Pricing Section (Common for both CREATE and EDIT modes) -->
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div v-if="minPriceAsPercentage"
                            ...
                            <div>
                                <span class="text-xs text-indigo-600 dark:text-indigo-400">{{ $t('pricing.min_price') }}:</span>
        ...
        <template #footer>
            ...
            <button @click="submitForm" :disabled="form.processing || minPriceValidationError || (!service?.id && !selectedCatalogPackageId)"
...
// Selected catalog package ID
const selectedCatalogPackageId = ref('');

const selectedCatalogPackage = computed(() =>
    props.otherBranchesPackages.find(p => p.id === selectedCatalogPackageId.value) ?? null
);

// Watch for pendingCatalogPackageId
watch(
    () => [props.pendingCatalogPackageId, props.otherBranchesPackages],
    ([pendingId]) => {
        if (pendingId && !props.service?.id) {
            const found = props.otherBranchesPackages.find(p => p.id === pendingId);
            if (found) {
                selectedCatalogPackageId.value = pendingId;
            }
        }
    },
    { deep: false }
...
// Copy catalog package details into form when selected
watch(selectedCatalogPackageId, id => {
    const pkg = props.otherBranchesPackages.find(p => p.id === id);
    if (pkg) {
        form.name_ar = pkg.name_ar;
        form.name_en = pkg.name_en;
        form.description_ar = pkg.description_ar || '';
        form.description_en = pkg.description_en || '';
        form.type = 'package';
        form.allow_price_override = pkg.allow_price_override;
        form.items = pkg.items.map(item => ({
            id: item.id,
...
// New item state (only for edit mode)
const newItem = ref({
    id: null,
    quantity: 1
});
...
watch(() => props.show, (open) => {
    if (open && !props.service) {
        ...
        selectedCatalogPackageId.value = '';
    }
});

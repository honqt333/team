    ...
    >
        ...
        <div v-else-if="invoices.length" class="divide-y divide-gray-100 dark:divide-gray-700/50">
            ...
            >
                <!-- Icon -->
                <div
                    class="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center flex-shrink-0"
                >
                    <svg
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                        <span
                            class="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors truncate"
                        >
                            {{ inv.customer?.name || '—' }}
                        </span>
                ...
                <div class="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <span
                        :class="[
                            ...
                            statusConfig[inv.payment_status]?.color,
                        ]"
                    >
                        {{
                            $t('invoices.payment_status.' + inv.payment_status) ||
                            (isRtl
                                ? statusConfig[inv.payment_status]?.label_ar
                                : statusConfig[inv.payment_status]?.label_en)
                        }}
                    </span>

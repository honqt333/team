...
const statusConfig = {
    ...
    ready_for_qc: {
        ...
        color: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300',
        dot: 'bg-emerald-500',
    },
    cancelled: {
        color: 'bg-rose-50 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300',
        dot: 'bg-rose-500',
    },
};
...
<template>
    <div
        class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-sm"
    >
        <!-- Header -->
        <div
            class="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-700/60"
        >
            <div>
                <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                    {{ $t('dashboard.recent_work_orders') }}
                </h3>
                <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                    {{ $t('dashboard.latest_8') }}
                </p>
            </div>
            <a
                href="/app/work-orders"
                class="text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 flex items-center gap-1 transition-colors"
            >
                {{ $t('common.view_all') }}
                <svg
                    class="w-3.5 h-3.5"
        ...
        <div v-else-if="workOrders.length" class="divide-y divide-gray-100 dark:divide-gray-700/50">
            ...
            >
                <!-- Avatar -->
                <div
                    class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
                >
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2">
                        <span
                            class="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate"
                        >
                            {{ wo.customer?.name || '—' }}
                        </span>
                        <span
                            class="text-xs text-gray-400 dark:text-gray-500 font-mono flex-shrink-0"
                        >
                            {{ wo.code }}
                        </span>
                    </div>

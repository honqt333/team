...
<template>
    <Teleport to="body">
        <Transition name="fade-backdrop">
            <div v-if="modelValue" class="fixed inset-0 z-[9999] flex" :dir="isRtl ? 'rtl' : 'ltr'">
                <!-- Backdrop -->
                ...
                <Transition :name="isRtl ? 'slide-rtl' : 'slide-ltr'">
                    ...
                    >
                            ...
                            <TransitionGroup name="list" tag="div" class="space-y-2 relative">
                                <div
                                    ...
                                    :class="[
                                        ...
                                        draggedIndex === index
                                            ? 'opacity-30 ring-2 ring-indigo-400 shadow-lg scale-[1.02] bg-white dark:bg-gray-800'
                                            : 'transition-colors duration-200',
                                        element.visible && draggedIndex !== index
                                            ? 'bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm'
                                            : draggedIndex !== index
                                              ? 'bg-gray-50 dark:bg-gray-800/40 border border-dashed border-gray-200 dark:border-gray-700/50 opacity-50'
                                              : '',
                                    ]"
                                >
                                    <!-- Drag handle icon -->
                                    <div
                                        class="flex-shrink-0 text-gray-300 dark:text-gray-600 p-1 pointer-events-none"
                                    >
                                        <svg
                                            class="w-4 h-4"
                                            fill="currentColor"
                                    ...
                                    <!-- Icon -->
                                    <div
                                        class="w-7 h-7 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center flex-shrink-0 pointer-events-none"
                                    >
                                        <span
                                            v-safe-html="element.icon"
                                            class="text-gray-500 dark:text-gray-400 [&>svg]:w-3.5 [&>svg]:h-3.5"
                                        />
                                    </div>

                                    <!-- Label -->
                                    <span
                                        class="flex-1 text-sm font-medium text-gray-700 dark:text-gray-300 truncate pointer-events-none"
                                    >
                                        {{ element.label }}
                                    </span>
                        ...
                        <!-- Footer -->
                        <div
                            class="px-4 py-4 border-t border-gray-200 dark:border-gray-700 flex gap-2"
                        >
                            <button
                                @click="reset"

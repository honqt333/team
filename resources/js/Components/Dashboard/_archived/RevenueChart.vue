...
const maxDay = computed(() =>
    props.data.reduce((m, d) => (d.total > m.total ? d : m), { total: 0, label: '' })
);
...
const chartOptions = computed(() => ({
    ...
    tooltip: {
        ...
        y: {
            formatter: (val) =>
                `${val.toLocaleString('en-SA', { minimumFractionDigits: 0, maximumFractionDigits: 0 })} ${props.currency}`,
        },
        x: { show: true },
    },
    grid: {
...
<template>
    ...
    >
        ...
        <div
            v-if="loading"
            class="h-48 mx-4 mb-4 bg-gray-100 dark:bg-gray-700/50 rounded-xl animate-pulse"
        />

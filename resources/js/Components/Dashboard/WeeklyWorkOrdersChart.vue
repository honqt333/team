...
const series = computed(() => [{ name: isRtl.value ? 'أوامر العمل' : 'Work Orders', data: props.data.map(d => d.count) }]);
</script>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">{{ $t('dashboard.last_7_days') }}</p>
            </div>
            <div class="flex items-center gap-2">
                <span class="text-2xl font-bold text-gray-900 dark:text-white">{{ total }}</span>
                <span class="text-xs text-gray-400 dark:text-gray-500">{{ $t('dashboard.total') }}</span>
            </div>
        </div>

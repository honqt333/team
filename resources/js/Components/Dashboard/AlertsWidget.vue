...
<template>
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-sm overflow-hidden">
        <!-- Header -->
        <div class="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
            ...
            <div>
                <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $t('dashboard.alerts_title') }}</h3>
                <p class="text-xs text-gray-400 dark:text-gray-500">
                    {{ hasAlerts ? $t('dashboard.alerts_attention', { n: alerts.length }) : $t('dashboard.all_clear') }}
                </p>
            </div>
            <div v-if="hasAlerts" class="ms-auto">
                <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-rose-500 text-white text-xs font-bold animate-pulse">
                    {{ alerts.length }}
                </span>
            </div>
        </div>
        ...
        <div v-if="loading" class="p-4 space-y-3">
            <div v-for="i in 2" :key="i" class="h-16 bg-gray-100 dark:bg-gray-700/50 rounded-xl animate-pulse" />
        </div>
        ...
        <div v-else-if="hasAlerts" class="p-4 space-y-3">
            ...
            >
                ...
                <div :class="['px-2.5 py-1 rounded-full text-white text-xs font-bold flex-shrink-0', alert.badgeBg]">
                    {{ alert.count }}
                </div>
            </a>
        </div>
        ...
        <div v-else class="py-8 px-5 text-center">
            ...
            <p class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ $t('dashboard.everything_looks_great') }}</p>
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ $t('dashboard.no_alerts_attention') }}</p>
        </div>
    </div>
</template>

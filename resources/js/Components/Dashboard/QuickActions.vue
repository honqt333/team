...
const actions = computed(() => {
    ...
    return allActions.filter(function(a) { return !a.permission || can(a.permission); });
});
</script>
...
<template>
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700/60">
            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ $t('dashboard.quick_actions') }}</h3>
            <p class="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{{ $t('dashboard.quick_actions_subtitle') }}</p>
        </div>

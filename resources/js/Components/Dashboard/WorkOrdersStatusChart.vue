...
const entries = computed(() =>
    Object.entries(props.data)
        ...
        .map(([status, count]) => ({
            status,
            count,
            config: statusConfig[status] || { label_ar: status, label_en: status, color: '#9ca3af' },
        }))
...
const labels = computed(() => entries.value.map(e => isRtl.value ? e.config.label_ar : e.config.label_en));
...
<template>
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700/60 shadow-sm p-5">
        <div class="flex items-center justify-between mb-4">
            <div>
                <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    {{ $t('dashboard.work_orders_status') }}
                </h3>
                <p class="mt-0.5 text-xs text-gray-400 dark:text-gray-500">{{ $t('dashboard.this_month') }}</p>
            </div>
                ...
                <div class="flex-1 space-y-2 min-w-0">
                    ...
                    >
                        <div class="flex items-center gap-2 min-w-0">
                            <span class="w-2.5 h-2.5 rounded-full flex-shrink-0" :style="{ background: entry.config.color }" />
                            <span class="text-xs text-gray-600 dark:text-gray-400 truncate">
                                {{ $t('work_orders.status.' + entry.status) || (locale === 'ar' ? entry.config.label_ar : entry.config.label_en) }}
                            </span>
                        </div>
                        <div class="flex items-center gap-2 flex-shrink-0">
                            <span class="text-xs font-semibold text-gray-800 dark:text-gray-200">{{ entry.count }}</span>

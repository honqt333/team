            ...
            >
                ...
                <div class="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                    <div class="flex items-center gap-2">
                        ...
                        <!-- Part Number -->
                        <span class="font-mono text-sm text-gray-600 dark:text-gray-400">{{ part.part_number || part.sku || (index + 1) }}</span>
                    </div>
                ...
                <div class="p-4 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-sm">
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">{{ $t('quotes.parts_display.price') }}:</span>
                            <span class="ms-1 font-medium text-gray-700 dark:text-gray-300">{{ formatCurrency(part.unit_price) }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">{{ $t('quotes.parts_display.qty') }}:</span>
                            <span class="ms-1 font-medium text-gray-700 dark:text-gray-300">{{ part.qty }} {{ getUnitName(part) }}</span>
                        </div>
                        <div>
                            <span class="text-gray-500 dark:text-gray-400">{{ $t('quotes.parts_display.discount') }}:</span>
                            <span class="ms-1 font-medium" :class="part.discount > 0 ? 'text-red-500' : 'text-gray-400'">
                        ...
                        <div v-if="showVat">
                            ...
                            <span class="ms-1 font-medium text-gray-700 dark:text-gray-300">{{ formatCurrency(part.tax_amount || 0) }}</span>
                        </div>
                    </div>

                    <!-- Linked Service -->
                    <div v-if="part.quote_line || part.service" class="pt-2 border-t border-gray-100 dark:border-gray-700">
                ...
                <div v-if="!readOnly" class="flex items-center justify-end gap-1 px-4 py-2 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-100 dark:border-gray-700 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                            ...
                            <th class="px-4 py-3 text-start text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.part_number') }}</th>
                            <th class="px-4 py-3 text-start text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.supplier') }}</th>
                            <th class="px-4 py-3 text-start text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.service') }}</th>
                            <th class="px-4 py-3 text-end text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.price') }}</th>
                            <th class="px-4 py-3 text-end text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.discount') }}</th>
                            <th class="px-4 py-3 text-center text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.qty') }}</th>
                            <th class="px-4 py-3 text-end text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.amount') }}</th>
                            <th v-if="showVat" class="px-4 py-3 text-end text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.vat') }}</th>
                            <th class="px-4 py-3 text-end text-gray-500 dark:text-gray-400 font-medium">{{ $t('quotes.parts_display.total') }}</th>
                            <th v-if="!readOnly" class="px-4 py-3 text-center text-gray-500 dark:text-gray-400 font-medium"></th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-400 max-w-[150px] truncate">
                                {{ part.quote_line?.description || getName(part.quote_line?.service) || getName(part.service) || $t('quotes.parts_display.without_service') }}
                            </td>
                            <td class="px-4 py-3 text-end font-mono text-gray-700 dark:text-gray-300">
                                {{ formatCurrency(part.unit_price) }}
                            </td>
                            <td class="px-4 py-3 text-end font-mono" :class="part.discount > 0 ? 'text-red-500' : 'text-gray-400'">
                            ...
                            <td class="px-4 py-3 text-end font-bold font-mono text-gray-900 dark:text-white">
                                {{ formatCurrency(part.total || (part.unit_price * part.qty) - (part.discount || 0) + (part.tax_amount || 0)) }}
                            </td>
                    ...
                    <tfoot class="bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                        <tr class="font-bold">
                            <td colspan="4" class="px-4 py-3 text-gray-700 dark:text-gray-300">{{ $t('common.total') }}</td>
                            <td class="px-4 py-3 text-end font-mono text-gray-700 dark:text-gray-300">{{ formatCurrency(totalPrice) }}</td>
                            <td class="px-4 py-3 text-end font-mono text-red-500">{{ formatCurrency(totalDiscount) }}</td>
                            <td class="px-4 py-3 text-center text-gray-700 dark:text-gray-300">{{ totalQty }}</td>
                            <td class="px-4 py-3 text-end font-mono text-gray-700 dark:text-gray-300">{{ formatCurrency(totalAmount) }}</td>
                            <td v-if="showVat" class="px-4 py-3 text-end font-mono text-gray-700 dark:text-gray-300">{{ formatCurrency(totalVat) }}</td>
                            <td class="px-4 py-3 text-end font-mono text-lg text-indigo-600 dark:text-indigo-400">{{ formatCurrency(grandTotal) }}</td>
                            <td v-if="!readOnly"></td>
                        </tr>
                    </tfoot>
                </table>

        ...
        <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <div class="bg-gray-50 dark:bg-gray-900/40 rounded-3xl p-6 space-y-5 border border-gray-100 dark:border-gray-800">
                ...
                <div v-if="form.source === 'warehouse' && !selectedPart" class="relative">
                    <div v-if="warehouses.length > 1">
                        ...
                        <SearchableSelect
                            ...
                            :placeholder="$t('inventory.parts.choose_warehouse')"
                            class="w-full mb-4"
                        />
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="block text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest ms-1">
                            VAT ({{ toEnglish(workOrder.tax_rate_snapshot) }}%)
                        </label>
                ...
                <div v-if="showServiceSelect">
                    ...
                    <SearchableSelect v-model="form.work_order_item_id" :options="serviceOptions" option-label="label"
                        :disabled="isReadOnly"
        ...
        <template #footer>
            <div class="flex items-center justify-between w-full">
                ...
                <button v-if="!isReadOnly && !props.part" type="button" @click="submitForm(false)"
                    :disabled="form.processing || !form.name || !form.qty || (form.unit_price === '' || form.unit_price === null) || (form.source === 'warehouse' && !form.warehouse_id) || (form.source !== 'warehouse' && !form.unit_id)"
...
function submitForm(shouldClose = true) {
    ...
    if (props.pendingMode) {
        emit('saved', { ...form.data(), total, isPending: true }, { close: shouldClose });
    ...
    const itemId = props.workOrderItemId || props.part?.work_order_item_id;

    if (itemId) {
        url = props.part 
            ? route('work-orders.items.parts.update', { work_order: props.workOrder.id, item: itemId, part: props.part.id })
            : route('work-orders.items.parts.store', { work_order: props.workOrder.id, item: itemId });
    } else {
        url = props.part

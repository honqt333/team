<template>
    ...
    >
        <!-- aria-live announces loading → result transitions to screen readers
             without hijacking focus. Use "polite" so the user is not
        ...
        <div class="wo-suggestions__body" role="region" aria-live="polite" aria-atomic="false">
            <!-- Loading skeleton (only on initial load when we have nothing to show) -->
            <div
            ...
            >
                ...
                >
                    ...
                    >
                        <span v-if="s.department_name" class="wo-suggestions__dept-badge">
                            {{ s.department_name }}
                        </span>
                        <span v-if="s.department_name" aria-hidden="true">•</span>
...
/**
 * WorkOrderSuggestionsPanel — Track A feature (ai-service-suggester).
 *
 * Renders a card-style list of AI-generated service / part suggestions
 * driven by the customer's complaint. The heavy lifting (debounced fetch,
 * error handling, stale-state tracking) lives in the composable; this
 * component is purely presentational + thin event emission.
 *
 * Props (per contract §9):
 *   - workOrder {Object, required} — Inertia prop. The composable reads
 *     `workOrder.id` (for the URL) and `workOrder.customer_complaint`
 *     (for the watch source).
 *   - endpoint {String, optional} — override the default URL. Useful in
 *     storybook / tests.
 *   - compact {Boolean, optional} — flag for future layout variants.
 *
 * Emits (per contract §9):
 *   - add {Object} — payload describing the chosen suggestion. Schema:
 *       { itemType: 'service' | 'part',
 *         itemId:   Number,
 *         name:     String,
 *         qty:      Number,
 *         departmentId: Number,
 *         unitPrice: Number /* in major units (not cents) *\/ }
 *   - error {Object} — payload with `{ message: String }`. Parents can
 *     toaster this if they want.
 */
...
// ─── Formatting ──────────────────────────────────────────────────────
/**
 * Convert cents + currency into a localized display string. The
 * suggestion payload already gives us cents and a per-item currency
 * code, so we use Intl.NumberFormat directly.
 */
...
/**
 * Emit the `add` event with the contract-shaped payload. Parent pages
 * route this into either the service-add or part-add modal.
 */
...
<style scoped>
/* ============================================================
 * WorkOrderSuggestionsPanel — token-driven styles. No hard-coded
 * colors. All values come from resources/js/Design/tokens.css.
 * ============================================================ */
.wo-suggestions {

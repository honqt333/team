                    ...
                    <input
                        ...
                        :class="
                            form.errors.name
                                ...
                                : !form.name
                                  ? 'border-red-300 dark:border-red-800 focus:ring-indigo-500'
    ...
    <BaseModal :show="showDuplicateModal" @close="showDuplicateModal = false" size="md">
        <template #title>
            <div class="flex items-center gap-3 text-amber-600 dark:text-amber-400">
                ...
                <span>
                    {{ $t('customers.validation.phone_taken_title') || 'العميل موجود مسبقاً!' }}
                </span>
            </div>
        </template>
        ...
        <div class="space-y-4 p-1">
            <p class="text-sm text-gray-600 dark:text-gray-400 leading-relaxed">
                {{
                    $t('customers.validation.phone_taken_desc') ||
                    'العميل الذي يحمل رقم الهاتف هذا مسجل مسبقاً بالنظام. يمكنك الضغط على بطاقة العميل أدناه للانتقال لملفه مباشرة:'
                }}
            </p>

            <div
                v-if="duplicateCustomer"
            ...
            >
                ...
                <div class="relative z-10 flex items-center gap-4">
                    <div
                        class="w-12 h-12 rounded-xl bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0"
                    >
                    ...
                    <div class="flex-1 min-w-0">
                        <h4
                            class="text-base font-bold text-gray-900 dark:text-white truncate group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors"
                        >
                            {{ duplicateCustomer.name }}
                        </h4>
                        <div
                            class="flex flex-wrap gap-x-3 gap-y-1 mt-1 text-xs text-gray-500 dark:text-gray-400"
                        >
                            <span class="font-mono" dir="ltr">
                                📞 {{ duplicateCustomer.phone }}
                            </span>
                            <span v-if="duplicateCustomer.email">
                                ✉️ {{ duplicateCustomer.email }}
                            </span>
                            <span v-if="duplicateCustomer.tax_number">
                                📋 {{ $t('customers.form.tax_number') }}:
                                {{ duplicateCustomer.tax_number }}
                            </span>
                        </div>
                    </div>
                    <div
                        class="text-indigo-600 dark:text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
                    >
                        <svg
...
const checkPhoneExistence = async () => {
    ...
    try {
        ...
        if (response.data.exists) {
            ...
            form.errors.phone =
                t('customers.validation.phone_taken') || 'رقم الهاتف هذا مسجل مسبقاً!';
        } else {
            duplicateCustomer.value = null;
            if (
                form.errors.phone ===
...
function submitForm() {
    ...
    const url = props.customer ? `/app/customers/${props.customer.id}` : '/app/customers';
    ...
    form[method](url, {
        ...
        onSuccess: (page) => {
            ...
            const savedCustomer =
                page.props.customer || page.props.flash?.customer || props.customer || form.data();

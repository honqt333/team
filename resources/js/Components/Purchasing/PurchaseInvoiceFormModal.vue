        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="space-y-4">
                    ...
                    <table class="w-full text-sm">
                        <thead class="bg-gray-50 dark:bg-gray-900/50 text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-center font-medium">{{ $t('inventory.parts.part_number') }}</th>
                                ...
                                <th class="px-4 py-3 text-center w-20">{{ $t('common.actions') }}</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                            <tr v-for="(item, index) in form.items" :key="index"
                                ...
                                <td class="px-4 py-3 text-center">
                                    <div class="font-medium text-gray-900 dark:text-white">{{ item.part?.name_ar }}</div>
                                ...
                                <td class="px-4 py-3 text-center" dir="ltr" lang="en" style="font-family: 'Inter', system-ui, sans-serif !important;">
                                    <span class="inline-flex items-center justify-center px-2 py-1 rounded-md bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 font-mono font-bold text-xs">
                                        {{ toEnglish(item.qty) }}
                                    </span>
                                </td>
                ...
                <div v-if="form.items.length > 0" class="flex justify-end">
                    <div class="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-4 w-full sm:w-1/3 border border-gray-200 dark:border-gray-700 space-y-2">
                        <div class="flex justify-between items-center text-sm text-gray-600 dark:text-gray-400">
                            <span>{{ $t('common.total') }}</span>
                            <span class="font-mono" dir="ltr" lang="en">{{ formatCurrency(itemsTotal) }}</span>
                        </div>
                        <div v-if="form.tax_included" class="flex justify-between items-center text-sm text-gray-500 dark:text-gray-550 text-xs">
            ...
            <div class="flex justify-end pt-2">
                <div class="bg-blue-50 dark:bg-blue-900/20 px-6 py-4 rounded-xl border border-blue-100 dark:border-blue-800 text-end">
                    <span class="block text-sm text-blue-600 dark:text-blue-400 mb-1 font-medium uppercase tracking-wide">
            ...
            <div>
                ...
                <textarea v-model="form.notes" rows="3"
                    class="w-full px-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"></textarea>
            </div>
        </form>
...
const submit = () => {
    ...
    if (remainingBalance.value > 0.01 && !form.create_credit_invoice) {
        form.setError('payments', t('purchasing.orders.errors.full_payment_required') || 'Full payment is required or credit option must be enabled.');
        return;
    }

    submitting.value = true;

    const url = route('app.invoices.purchases.store');

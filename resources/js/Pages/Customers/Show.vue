            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex items-center gap-4 flex-1">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-sm text-gray-500 dark:text-gray-400">
                                    <span v-if="(customer.type === 'company' || customer.type === 'government') && customer.contact_name" class="flex items-center gap-1 text-indigo-600 dark:text-indigo-400 font-medium whitespace-nowrap">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/>
                                        </svg>
                                        {{ customer.contact_name }}
                                    </span>
                                    <a :href="`tel:${customer.phone}`" class="flex items-center gap-1 whitespace-nowrap hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors" dir="ltr">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                                        </svg>
                                        {{ customer.phone }}
                                    </a>
                                    <a v-if="customer.email" :href="`mailto:${customer.email}`" class="flex items-center gap-1 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-6">
                    ...
                    <div v-if="activeTab === 'vehicles'" class="space-y-4">
                        ...
                        <!-- Grid View -->
                        <div v-if="vehicleViewMode === 'grid' && filteredVehicles.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                            <div
                                    ...
                                    <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700/50 flex items-center justify-between bg-gradient-to-r from-gray-50/50 to-transparent dark:from-gray-900/50">
                                        ...
                                        <div class="flex items-center gap-2">
                                            <div class="bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg px-2 py-0.5 shadow-sm">
                                                <span class="text-base font-bold text-gray-900 dark:text-gray-100 tracking-wider font-mono">
                                    ...
                                    <div class="p-4 flex-1">
                                        <div class="mb-3">
                                            <div class="flex items-baseline justify-between mb-1 gap-1">
                                                ...
                                                <span v-if="vehicle.year" class="px-1.5 py-0.5 text-[10px] font-bold bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded-md">
                                                    {{ vehicle.year }}
                                                </span>
                                            </div>
                                        ...
                                        <!-- Odometer & VIN Footer -->
                                        <div class="text-[10px] text-gray-500 dark:text-gray-400 pt-2 border-t border-gray-100 dark:border-gray-700/50 flex flex-col gap-1.5 mt-auto">
                                            <div v-if="vehicle.odometer" class="flex items-center gap-1.5">
                                                <svg class="w-3.5 h-3.5 text-teal-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                                <span class="font-medium text-gray-700 dark:text-gray-300">{{ vehicle.odometer?.toLocaleString() }}</span>
                                                <span>{{ $t('common.km') }}</span>
                                            </div>
                                            <div v-if="vehicle.vin" class="flex items-center gap-1.5">
                                                <svg class="w-3.5 h-3.5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/></svg>
                                                <span class="font-mono text-[9px] truncate max-w-[80px]" :title="vehicle.vin">{{ vehicle.vin }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>

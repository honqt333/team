<template>
    <div>
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {{ $t('system_settings.models.subtitle') }}
                </p>
            </div>
            <button
                @click="openModal()"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="model in modelsData" :key="model.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                        <td class="px-4 py-3 text-sm text-gray-900 dark:text-white">
                            <span class="flex items-center gap-2">
                                {{ model.name_ar }}
                                <span v-if="model.source === 'system'" class="inline-flex items-center px-1.5 py-0.5 text-[10px] font-medium rounded bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300">
                                    {{ $t('common.system') }}
                                </span>
                            </span>
                        </td>
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                            {{ model.updated_by ? model.updated_by.name : '—' }}
                        </td>
                        <td class="px-4 py-3 text-center">
        ...
        <!-- Pagination -->
        <div v-if="models?.links" class="mt-4 flex justify-center gap-1">
...
const openModelModal = inject('openModelModal');

const search = ref(props.filters?.search || '');
const selectedMake = ref(props.filters?.make_id || '');

const modelsData = computed(() => {
    return props.models?.data || [];
});

const makesForFilter = computed(() => {
...
function handleSearch() {
    router.get('/app/settings/models', { search: search.value, make_id: selectedMake.value }, { preserveState: true });
}
...
function toggleActive(model) {
    router.patch(`/app/settings/models/${model.id}/toggle-active`);
}

async function handleDelete(model) {
    const confirmed = await confirm({
        title: t('common.confirm_delete_title'),
        message: `${getName(model)}: ${t('common.confirm_delete_message')}`,
    ...
    if (confirmed) {
        router.delete(`/app/settings/models/${model.id}`);
    }
}
</script>

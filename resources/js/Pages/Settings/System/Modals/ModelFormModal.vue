        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    {{ model ? $t('system_settings.models.edit') : $t('system_settings.models.add') }}
                </h3>
                <button
                    @click="$emit('close')"
...
const form = useForm({
    ...
    name_en: props.model?.name_en || '',
    make_id: props.model?.make_id || '',
    is_active: props.model?.is_active ?? true,
});

function submit() {
    const url = props.model 
        ? `/app/settings/models/${props.model.id}` 
        : '/app/settings/models';

        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                    {{ color ? $t('system_settings.colors.edit') : $t('system_settings.colors.add') }}
                </h3>
                <button
                    @click="$emit('close')"
            ...
            <form @submit.prevent="submit" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                        {{ $t('system_settings.form.hex_code') }}
                    </label>
                    <div class="flex items-center gap-3">
                        <input
                            v-model="form.hex_code"
                            type="color"
                            class="w-12 h-10 p-1 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer"
                        />
                        <input
                            v-model="form.hex_code"
                            type="text"
                    ...
                    <p v-if="form.errors.hex_code" class="mt-1 text-sm text-red-500">{{ form.errors.hex_code }}</p>
                </div>
...
const form = useForm({
    ...
    hex_code: props.color?.hex_code || '#000000',
    is_active: props.color?.is_active ?? true,
});

function submit() {
    const url = props.color 
        ? `/app/settings/colors/${props.color.id}` 
        : '/app/settings/colors';
    
    const method = props.color ? 'put' : 'post';

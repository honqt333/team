        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                    {{ incomeCategory ? $t('system_settings.income_categories.edit') : $t('system_settings.income_categories.add') }}
                </h3>
                <button
                    @click="$emit('close')"
            ...
            <form @submit.prevent="submit" class="p-6 space-y-4">
                ...
                <div>
                    <SearchableSelect
                        ...
                        option-label="label"
                        option-value="value"
                        :label="$t('system_settings.form.transaction_type')"
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <button
                        ...
                        class="px-4 py-2 text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 rounded-lg transition-all disabled:opacity-50"
                    >
                        {{ $t('common.save') }}
                    </button>
                </div>
...
const form = useForm({
    name_ar: props.incomeCategory?.name_ar || '',
    name_en: props.incomeCategory?.name_en || '',
    transaction_type: props.incomeCategory?.transaction_type || 'revenue',
    is_active: props.incomeCategory?.is_active ?? true,
});

function submit() {
    const url = props.incomeCategory 
        ? `/app/settings/income-categories/${props.incomeCategory.id}` 
        : '/app/settings/income-categories';
    
    const method = props.incomeCategory ? 'put' : 'post';

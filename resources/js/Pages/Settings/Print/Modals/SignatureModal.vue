        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-3xl text-start overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-gray-200 dark:border-gray-700">
                ...
                <div class="bg-white dark:bg-gray-800 px-8 py-5 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p v-if="docKey" class="text-xs text-gray-400">{{ $t('print_settings.' + docKey) }}</p>
                        </div>
                    </div>
                    <button type="button" @click="$emit('close')" class="w-8 h-8 rounded-full flex items-center justify-center text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-all">
                ...
                <div class="px-8 pt-5 border-b border-gray-100 dark:border-gray-700">
                    <div class="flex items-center gap-1 bg-gray-50/60 dark:bg-gray-900/40 p-1 rounded-xl" role="tablist">
                        <button
                            v-for="tab in tabs"
                            :key="tab.key"
                            type="button"
                ...
                <div class="px-8 py-6 space-y-5 max-h-[60vh] overflow-y-auto custom-scrollbar">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div class="space-y-1.5">
                            ...
                            <input
                                ...
                                type="text"
                                :placeholder="isRtl ? 'مثال: توقيع المدير' : 'e.g. Manager Signature'"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input
                                ...
                                type="text"
                                :placeholder="isRtl ? 'مثال: Manager Signature' : 'e.g. Manager Signature'"
                    ...
                    <div v-show="activeTab === 'draw'" class="space-y-3">
                        ...
                        <!-- Toolbar -->
                        <div class="flex items-center justify-between gap-3 p-2.5 rounded-xl bg-gray-50/60 dark:bg-gray-900/40 border border-gray-100 dark:border-gray-700">
                            <div class="flex items-center gap-2 flex-1 min-w-0">
                                <label class="text-[10px] font-black text-gray-400 uppercase tracking-widest whitespace-nowrap">
                                ...
                                />
                                <span class="text-xs font-mono text-gray-600 dark:text-gray-300 w-6 text-center">{{ brushSize }}</span>
                            </div>
                            <button
                                type="button"
                                @click="clearCanvas"
                        ...
                        <!-- Canvas -->
                        <div class="rounded-2xl border-2 border-dashed border-amber-200 dark:border-amber-800/50 bg-white dark:bg-gray-900/30 p-2 flex items-center justify-center">
                            <canvas
                                ref="canvasEl"
                                width="640"
                                height="240"
                        ...
                        <p v-if="canvasError" class="text-[10px] text-red-500 font-bold">{{ canvasError }}</p>
                    </div>
                    ...
                    <div v-show="activeTab === 'upload'" class="space-y-3">
                        ...
                        >
                            ...
                            <p class="text-sm font-bold text-indigo-600 dark:text-indigo-400">
                                {{ uploadFile ? uploadFile.name : (isRtl ? 'اضغط لاختيار ملف' : 'Click to choose a file') }}
                            </p>
                        ...
                        <div v-if="uploadPreviewUrl" class="rounded-2xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/30 p-3 flex items-center justify-center">
                            <img :src="uploadPreviewUrl" alt="preview" class="max-h-32 max-w-full object-contain" />
                        </div>

                        <p v-if="uploadError" class="text-[10px] text-red-500 font-bold">{{ uploadError }}</p>
                    </div>
                    ...
                    <div v-show="activeTab === 'library'" class="space-y-3">
                        ...
                        <div v-if="librarySignatures.length === 0" class="rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700 bg-gray-50/40 dark:bg-gray-900/30 p-8 flex flex-col items-center gap-2 text-gray-400">
                            <svg class="w-10 h-10 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                            </svg>
                            <span class="text-xs font-bold">{{ $t('print_settings.library_empty') }}</span>
                        </div>
                        ...
                        <div v-else class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            <button
                                ...
                                :class="selectedLibraryId === sig.id
                                    ? 'border-emerald-500 ring-2 ring-emerald-500/20'
                                    : 'border-gray-200 dark:border-gray-700 hover:border-emerald-300 dark:hover:border-emerald-700'"
                            >
                                <div class="aspect-[8/3] bg-white flex items-center justify-center overflow-hidden rounded-xl">
                                    <img
                                        :src="sig.url"
                                ...
                                <p class="text-[10px] font-bold text-gray-700 dark:text-gray-300 truncate text-start">
                                    {{ isRtl ? (sig.name_ar || sig.name_en) : (sig.name_en || sig.name_ar) }}
                                </p>
                                <div v-if="selectedLibraryId === sig.id" class="absolute top-1.5 end-1.5 w-5 h-5 rounded-full bg-emerald-500 text-white flex items-center justify-center">
                    ...
                    <!-- Submission error -->
                    <p v-if="form.errors && (form.errors.signature || form.errors.error || form.errors.name_ar || form.errors.name_en)" class="text-[10px] text-red-500 font-bold">
                        {{ form.errors.signature || form.errors.error || form.errors.name_ar || form.errors.name_en }}
                    </p>
                </div>

                <!-- Footer -->
                <div class="bg-gray-50 dark:bg-gray-900/50 px-8 py-5 flex items-center gap-3 border-t border-gray-100 dark:border-gray-700">
                    <button
                        type="button"
                        @click="$emit('close')"
                        :disabled="uploading"

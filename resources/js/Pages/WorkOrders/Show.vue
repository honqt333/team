        ...
        <div class="space-y-6">
            ...
            >
                ...
                <WorkOrderSuggestionsPanel
                    :work-order="workOrder"
                    @add="handleAddSuggestion"
                    @error="handleSuggestionError"
                />
 ...
 * into the existing add flow pre-bound to the suggested item.
 *
 * - For services: open the service-add modal pre-bound to the suggested
 *   department and switch to the services tab.
 * - For parts: open the part-add modal pre-selected to the suggested part
 *   when we can resolve it from the catalog, and switch to the parts tab.
 *
 * The composable payload already includes `itemId`, so the modal can
 * pre-select the closest catalog match server-side.
 */
function handleAddSuggestion(payload) {
    if (!payload) return;

    // Switch to the relevant tab so the user sees the result.
    activeTab.value = payload.itemType === 'part' ? 'parts' : 'services';

    if (payload.itemType === 'part') {
        // Resolve the part catalog row so the modal can pre-fill it.
        const part = (props.inventoryParts || []).find(
            (p) => Number(p.id) === Number(payload.itemId)
        );
        if (part) {
            selectedPartToEdit.value = { ...part, _qty: payload.qty };
        }
        showAddPartModal.value = true;
        return;
    }

    // Default: services flow. openAddServiceModal takes a department id.
    openAddServiceModal(payload.departmentId);
}

/**
 * Surface AI suggestion errors through the existing toast system so the
 * user gets a non-blocking notification in addition to the inline alert.
 */
function handleSuggestionError(payload) {
    if (!payload || !payload.message) return;
    errorToast(t('work_orders.suggestions.error_generic'));
}

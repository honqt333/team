...
const documentSettings = computed(() => {
    ...
    const docSettings = tenantSettings?.documents?.['condition_report'] || {};
    return {
        title_ar: docSettings.title_ar || 'تقرير حالة المركبة',
        title_en: docSettings.title_en || 'Vehicle Condition Report',
        ...
        signatures: docSettings.signatures && docSettings.signatures.length > 0 ? docSettings.signatures : [
            { name_ar: 'توقيع المستلم' },
            { name_ar: 'توقيع العميل' }
        ]
    };
});

<template>
    <AppLayout :title="$t('nav.sales') + ' - ' + $t('purchasing.title')">
        <div class="space-y-6">

            <!-- Header Section -->
            <PageHeader :title="$t('nav.sales') + ' (' + $t('inventory.title') + ')'"
                :subtitle="$t('invoices.sales.subtitle')"
                :totalCount="invoices?.total ? toEnglish(invoices.total) : '0'"
                :countLabel="$t('invoices.sales.tab_invoices')" gradientFrom="from-blue-600" gradientTo="to-indigo-600"
                glowFrom="from-blue-500" badgeBg="bg-blue-50/50 dark:bg-blue-900/30"
                ...
                <template #actions>
                    <div class="flex items-center gap-4">
                        ...
                        <button v-if="can('crm.work_orders.create') || isAnyAdmin()" @click="showCreateModal = true"
                            class="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-bold rounded-2xl shadow-lg shadow-blue-500/20 hover:shadow-blue-500/35 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300">
                        ...
                        <!-- Actions Group -->
                        <div
                            class="flex items-center gap-1.5 p-1.5 bg-gray-50/50 dark:bg-gray-900/50 backdrop-blur-md rounded-2xl border border-gray-200/50 dark:border-gray-700/50 shadow-inner">
                            <div class="flex gap-1.5">
                                <button @click="setView('grid')" :title="$t('common.grid_view')" :class="[
                                    ...
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                            d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2z" />
                                    </svg>
                                </button>
                                <button @click="setView('list')" :title="$t('common.list_view')" :class="[
                                    'p-2.5 rounded-xl transition-all shadow-sm',
                                    viewMode === 'list'
                                        ? 'bg-blue-600 text-white shadow-blue-200 dark:shadow-none'
                                        : 'text-gray-400 hover:text-gray-600 hover:bg-white dark:hover:bg-gray-800'
                ...
                <template #filters>
                    <div class="flex flex-wrap items-end gap-3">
                        ...
                        <div class="relative group lg:w-48">
                            <div
                                class="absolute inset-y-0 start-0 ps-4 flex items-center pointer-events-none text-gray-400 group-focus-within:text-blue-500 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                </svg>
                            </div>
                            <input type="text" v-model="localFilters.search" :placeholder="$t('common.search')"
                        ...
                        <div class="lg:w-40">
                            ...
                            <SearchableSelect v-model="localFilters.payment_status" :options="statusOptions"
                                option-label="label" option-value="value" :placeholder="$t('invoices.all_statuses')"
            ...
            <div class="p-0">
                ...
                <div v-show="activeTab === 'invoices'">
                    ...
                    <div v-else-if="viewMode === 'grid'"
                        ...
                        <Link v-for="invoice in invoices.data" :key="invoice.id"
                            ...
                            <div class="p-5">
                                ...
                                <div class="flex items-start justify-between mb-4">
                                    <div>
                                        <span
                                            class="text-xs font-black text-blue-600 dark:text-blue-400 tracking-wide">#{{
                                            invoice.invoice_number }}</span>
                                        <p
                                            class="mt-1 text-sm font-bold text-gray-900 dark:text-white truncate max-w-[140px]">
                                            {{
                                ...
                                <div class="space-y-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                                    <div class="flex justify-between items-center text-xs">
                                        ...
                                        <span class="font-bold text-gray-900 dark:text-white" dir="ltr">{{
                                            formatCurrency(invoice.total_incl_tax) }}</span>
                                    </div>
                                    <div class="flex justify-between items-center text-xs">
                                        <span class="text-gray-500 dark:text-gray-400">{{ $t('invoices.balance')
                                            }}</span>
                                        <span
                                            :class="invoice.balance > 0 ? 'text-red-500 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'"
                                            class="font-bold" dir="ltr">{{ formatCurrency(invoice.balance) }}</span>
                                    </div>
                                    <div class="flex justify-between items-center text-xs">
                                        <span class="text-gray-500 dark:text-gray-400">{{ $t('invoices.issue_date')
                                            }}</span>
                                        <span class="text-gray-600 dark:text-gray-300">{{ formatDate(invoice.issue_date)
                                            }}</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <thead>
                                    <tr class="bg-gray-50/50 dark:bg-gray-900/80">
                                        ...
                                        <th
                                            ...
                                            {{ $t('common.status') }}</th>
                                        <th
                                            class="px-4 py-4 text-end text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.15em] align-middle text-center">
                                            {{ $t('common.actions') }}</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/30">
                                    <tr v-for="(invoice, index) in invoices.data" :key="invoice.id"
                                        ...
                                        <td class="px-4 py-4 text-center align-middle">
                                            <Link :href="route('app.invoices.show', invoice.id)"
                                        ...
                                        <!-- تاريخ الفاتورة -->
                                        <td
                                            class="px-4 py-4 text-center text-sm text-gray-600 dark:text-gray-300 font-mono align-middle">
                                        ...
                                        <td class="px-4 py-4 text-center align-middle">
                                            <p
                                                class="font-medium text-gray-900 dark:text-white truncate max-w-[180px] mx-auto">
                                                {{ invoice.customer_name_snapshot || invoice.customer?.name || '—' }}
                                            </p>
                                        </td>

                                        <!-- المجموع -->
                                        <td class="px-4 py-4 text-center text-sm font-bold text-gray-900 dark:text-white font-mono align-middle"
                                            dir="ltr">
                                            {{ formatCurrency(getGrossTotal(invoice)) }}
                                        </td>

                                        <!-- الخصم -->
                                        <td class="px-4 py-4 text-center text-sm font-medium text-rose-600 dark:text-rose-400 font-mono align-middle"
                                            dir="ltr">
                                        ...
                                        <!-- VAT -->
                                        <td class="px-4 py-4 text-center text-sm font-medium text-gray-500 dark:text-gray-400 font-mono align-middle"
                                            dir="ltr">
                                            {{ formatCurrency(invoice.total_tax) }}
                                        </td>

                                        <!-- مبلغ الفاتورة -->
                                        <td class="px-4 py-4 text-center text-sm font-black text-gray-900 dark:text-white font-mono align-middle"
                                            dir="ltr">
                                            {{ formatCurrency(invoice.total_incl_tax) }}
                                        </td>

                                        <!-- الباقي -->
                                        <td class="px-4 py-4 text-center text-sm font-bold font-mono align-middle" dir="ltr"
                                            :class="invoice.balance > 0 ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'">
                                        ...
                                        <td class="px-4 py-4 text-end align-middle">
                                            <div class="flex items-center justify-end gap-1">
                                                ...
                                                <Link :href="route('app.invoices.print', invoice.id)" target="_blank"
                                                    class="p-2 rounded-lg text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 transition-all">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                    ...
                    <!-- Pagination -->
                    <div v-if="invoices.links?.length > 3"
                        class="px-4 py-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30 flex items-center justify-between flex-wrap gap-3">
                        <span class="text-sm text-gray-500 dark:text-gray-400">
                            {{ $t('common.showing') }} {{ toEnglish(invoices.from) }} - {{ toEnglish(invoices.to) }} {{
                            $t('common.of') }} {{ toEnglish(invoices.total) }}
                        </span>
                ...
                <div v-show="activeTab === 'returns'" class="py-20 text-center">
                    <div
                        class="w-16 h-16 mx-auto rounded-2xl bg-red-50 dark:bg-red-900/20 flex items-center justify-center mb-4">
...
const applyFilters = () => {
    router.get(route('app.purchasing.sales.index'), {

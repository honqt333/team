    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="w-60" v-if="$page.props.auth.available_centers?.length > 1">
                <SearchableSelect :modelValue="selectedCenter"
                    @update:modelValue="value => emit('center-change', value)"
                    :options="[{ id: '', name: $t('common.all') }, ...centers]" option-label="name" option-value="id"
                    :label="$t('common.center')" :placeholder="$t('common.select')" />
            </div>

            <!-- Add Payment Button -->
            <button @click="openModal"
                class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 flex items-center gap-2"
                :class="{ 'ml-auto': !($page.props.auth.available_centers?.length > 1) }">
        ...
        <div
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-if="loading">
                            <td colspan="6" class="px-6 py-4 text-center text-gray-500">{{ $t('common.loading') }}...
                            </td>
                        </tr>
                        <template v-else-if="payments?.data?.length">
                            <tr v-for="payment in payments.data" :key="payment.id"
                                ...
                                <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                    <Link
                                        :href="route('app.hr.employees.show', { employee: payment.employee_id, tab: 'financial' })"
                                ...
                                <td class="px-6 py-4 flex items-center gap-2">
                                    <template v-if="payment.status === 'pending'">
                                        <button @click="approvePayment(payment)"
                                            class="text-blue-600 hover:text-blue-900" title="Approve">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <template v-else-if="payment.status === 'approved'">
                                        <button @click="payPayment(payment)"
                                            class="text-emerald-600 hover:text-emerald-900" title="Mark as Paid">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div v-if="payments?.meta"
                class="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex justify-center">
                <div class="flex gap-2">
                    <Link v-for="link in payments.meta.links" :key="link.label" :href="link.url || '#'"
        ...
        <BaseModal :show="showModal" @close="closeModal"
            :title="editMode ? 'تعديل دفعة' : $t('hr.financial.other_payments.add')">
            <form @submit.prevent="submit" class="space-y-4">
                <div>
                    <SearchableSelect v-model="form.employee_id" :options="employees" option-label="name_ar"
                        option-value="id" :label="$t('common.employee')" :placeholder="$t('common.select')"
                        :error="errors.employee_id?.[0]" required />
                </div>
                ...
                <div>
                    ...
                    <textarea v-model="form.notes"
                        class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-emerald-500 focus:ring-emerald-500 rounded-md shadow-sm"
                        rows="3"></textarea>

...
import LandingTab from './Tabs/LandingTab.vue';
...
import SeoSocialTab from './Tabs/SeoSocialTab.vue';
...
const tabs = [
  ...
  { id: 'footer',      name: 'التذييل',                  icon: 'M12 15l-4-4h8l-4 4z' },
...
const listKeys = ['website_header_menu', 'website_footer_menu', 'landing_banners_list',
  'landing_features_list', 'landing_faq_list', 'landing_testimonials_list',
  'landing_stats_list', 'website_custom_pages', 'website_social_links'];

function initSettings(src) {
  const out = {};
  for (const key of Object.keys(src)) {
    out[key] = listKeys.includes(key) ? (src[key] || []) : src[key];
  }
  return out;
}

const form = useForm({ settings: initSettings(props.settings) });
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col md:flex-row min-h-[75vh]">
        ...
        <div class="w-full md:w-64 bg-gray-50 dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700 flex-shrink-0">
          <nav class="space-y-0.5 p-3">
            <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id"
              :class="[
                activeTab === tab.id
                  ? 'bg-indigo-50 dark:bg-indigo-900/50 border-indigo-600 text-indigo-700 dark:text-indigo-300'
        ...
        <div class="flex-1 p-6 md:p-10 lg:p-12 overflow-y-auto max-h-[75vh] custom-scrollbar bg-white dark:bg-gray-800">
          <div class="max-w-4xl mx-auto space-y-12 animate-fade-in pb-12">
            ...
            <!-- 2. القوائم + 3. البنرات -->
            <ListsTab v-if="activeTab === 'menus' || activeTab === 'banners'" :form="form" :addItem="addItem" :removeItem="removeItem" :tab="activeTab" />

            <!-- 4~10. محتوى الواجهة الرئيسية -->
            <LandingTab v-if="activeTab === 'landing' || activeTab === 'features' || activeTab === 'pricing' || activeTab === 'stats' || activeTab === 'testimonials' || activeTab === 'faq' || activeTab === 'contact'" :form="form" :addItem="addItem" :removeItem="removeItem" :tab="activeTab" />

            <!-- 11. الوثائق والصفحات -->
            <PagesTab v-if="activeTab === 'pages'" :form="form" :addItem="addItem" :removeItem="removeItem" />

            <!-- 12. الصيانة + 13. التذييل + 14. الإعلانات -->
            <AdvancedTab v-if="activeTab === 'maintenance' || activeTab === 'footer' || activeTab === 'announcement'" :form="form" :tab="activeTab" />

            <!-- 15. التكاملات -->
            <IntegrationsTab v-if="activeTab === 'integrations'" :form="form" />

            <!-- 16. SEO + السوشيال -->
            <SeoSocialTab v-if="activeTab === 'seo_social'" :form="form" :addItem="addItem" :removeItem="removeItem" />
...
<!-- Shared utility classes (unscoped to reach child tab components) -->
<style>
.input-enterprise {
  @apply mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm py-2 px-3 bg-white dark:bg-gray-700 dark:text-white transition-colors duration-200;
}
textarea.input-enterprise { @apply py-3; }
...
.btn-outline {
  @apply inline-flex items-center px-3 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200;
}

.badge-lang {
  @apply inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold tracking-wider uppercase bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600;
}
</style>

    ...
    <AppLayout>
        <div class="space-y-4">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                <!-- Header Section -->
                <div
                    class="bg-gradient-to-r from-amber-500/10 via-orange-500/5 to-transparent dark:from-amber-900/30 dark:via-orange-900/10 p-6 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col lg:flex-row lg:items-center gap-6">
                        <!-- Vehicle Info -->
                        <div class="flex items-center gap-4 flex-1">
                            <!-- Vehicle Logo -->
                            <div
                                class="w-16 h-16 rounded-2xl bg-white dark:bg-gray-700 shadow-sm flex items-center justify-center overflow-hidden">
                                <img v-if="quote.vehicle?.make?.logo_path"
                                    :src="`/storage/${quote.vehicle.make.logo_path}`" :alt="getName(quote.vehicle.make)"
                ...
                <div
                    ...
                    <!-- Edit Button -->
                    <button v-if="quote.status === 'draft'" @click="openEditModal"
                    ...
                    <!-- Delete Button -->
                    <button v-if="quote.status === 'draft'" @click="deleteQuote"
                    ...
                    <div class="relative">
                        ...
                        <div v-if="showShareMenu"
                            ...
                            <button
                                class="w-full px-4 py-2 text-start text-sm text-gray-400 dark:text-gray-500 cursor-not-allowed flex items-center gap-2">
                ...
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="grid grid-cols-3 gap-4 text-center">
                        <div class="bg-gray-50 dark:bg-gray-900/50 rounded-xl p-3">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">{{ $t('quotes.show.subtotal') }}
                            </p>
                            <p class="text-lg font-bold text-gray-900 dark:text-white">{{ formatPrice(quote.subtotal) }}
                            </p>
                        </div>
                        <div class="bg-red-50 dark:bg-red-900/20 rounded-xl p-3">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">{{ $t('quotes.show.total_discount')
                                }}</p>
                            <p class="text-lg font-bold text-red-600 dark:text-red-400">{{
                                formatPrice(quote.total_discount) }}</p>
                        </div>
                        <div class="bg-green-50 dark:bg-green-900/20 rounded-xl p-3">
                            <p class="text-xs text-gray-500 dark:text-gray-400 mb-1">{{ $t('quotes.show.net_total') }}
                            </p>
                            <p class="text-lg font-bold text-green-600 dark:text-green-400">{{ formatPrice(quote.total)
                                }}</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div v-if="quote.customer_complaint"
                            ...
                            <div class="flex items-center gap-2 mb-2">
                                ...
                                <h4 class="font-semibold text-orange-700 dark:text-orange-300">{{
                                    $t('quotes.show.customer_complaint')
                                    }}</h4>
                            </div>
                            <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{
                                quote.customer_complaint }}</p>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div v-show="activeTab === 'services'" class="space-y-4">
                        ...
                        <div class="flex justify-end">
                            <div class="relative">
                                <button v-if="canEdit" @click="showDeptMenu = !showDeptMenu"
                        ...
                        <div v-if="displayDepartments.length > 0" class="space-y-3">
                            <div v-for="dept in displayDepartments" :key="dept.id"
                                ...
                                <!-- Department Content -->
                                <div v-show="expandedDepartments.includes(dept.id)"
                                    class="p-4 space-y-2 bg-gray-50/50 dark:bg-gray-900/30">
                                    <!-- Services List -->
                                    <div v-for="line in getDepartmentLines(dept.id)" :key="line.id"
                                        ...
                                        <div class="flex-1">
                                            <p class="font-medium text-gray-900 dark:text-white">{{ line.description ||
                                                getName(line.service) }}</p>
                                            <div class="flex items-center gap-4 mt-1 text-sm">
                                                <span class="text-gray-600 dark:text-gray-400">{{
                                                    formatPrice(line.unit_price) }}</span>
                                                <span v-if="line.discount_amount > 0"
                                        ...
                                        <div v-if="canEdit" class="flex items-center gap-1">
                                            <button @click="openEditServiceModal(line)"
                                            ...
                                            <button @click="deleteServiceLine(line)"
                                    ...
                                    <!-- Empty State -->
                                    <p v-if="getDepartmentLines(dept.id).length === 0"
                                        class="text-center text-gray-400 dark:text-gray-500 py-4 text-sm">
                                        {{ $t('quotes.show.no_services') }}
                                    </p>

                                    <!-- Add Service Button -->
                                    <button v-if="canEdit" @click="openAddServiceModal(dept.id)"
        ...
        <!-- Service Modal -->
        <QuoteServiceModal v-if="showServiceModal" :show="showServiceModal" :quote="quote" :line="selectedLine"
            :department-id="selectedDepartmentId" :services="departmentServices" @close="closeServiceModal"
            @saved="handleServiceSaved" />

        <!-- Edit Quote Modal -->
        <QuoteFormModal v-if="showEditModal" :show="showEditModal" :quote="quote" :customers="[]"
            :departments="departments" :makes="[]" :colors="[]" :modelsByMake="{}" @close="showEditModal = false"
...
// Computed for edit access
const canEdit = computed(() => {
    return can('quotes.update') &&
        props.quote.status !== 'converted' &&
        props.quote.status !== 'rejected';
});
